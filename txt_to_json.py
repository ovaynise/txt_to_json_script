...
def txt_to_json_with_uncodding(file_name: str):
    formating = ['.txt', '.json']
    file_txt = file_name+formating[0]
    file_json = file_name+formating[1]
    with open(file_txt, encoding='utf-8') as f:
        ...
        for line in f:
            ...
            line_words = [word.lower() for word in
                           line.strip().split() if word.isalpha()]
...
""" Введи название файла txt (без расширения) в 
папке со скриптом который хочешь перевести json.
...
"""
# Введи название файла txt  в папке со скриптом

txt_to_json_with_uncodding('пример')
# Введи название файла txt  в папке со скриптом

txt_to_json_with_codding('example')
